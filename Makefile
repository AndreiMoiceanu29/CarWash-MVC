...
REPO_FILES=FactorySettings.cpp
COMPILE_REPO_FILES=$(REPO_FILES:%.cpp=$(REPO_DIRECTORY)/%.cpp)
...
OBJECTS=$(MODEL_FILES:%.cpp=%.o) $(SERVICE_FILES:%.cpp=%.o) $(VIEW_FILES:%.cpp=%.o) $(VALIDATOR_FILES:%.cpp=%.o) $(REPO_FILES:%.cpp=%.o)
...
COMPILE_FILES=$(COMPILE_MODEL_FILES) $(COMPILE_SERVICE_FILES) $(COMPILE_VIEW_FILES) $(COMPILE_VALIDATOR_FILES) $(COMPILE_REPO_FILES) $(MAIN_FILE)

LINK=$(CC) $(COMPILE_OBJECT_FILES_MAIN) -o $(BUILD_DIRECTORY)/$(OUTPUT_FILE)

build_and_run_for_mem_leaks: clean_all build run_valgrind

run_valgrind:
	valgrind ./$(BUILD_DIRECTORY)/$(OUTPUT_FILE)
